...
Aether. Full prompt text for each stage lives in `PROMPTS.md`.

> Note: entries below reference internal working docs from the build
> (`PROJECT_STATUS.md`, `docs/API_CONTRACT.md`, `docs/DEPLOYMENT.md`,
> `docs/PROJECT_STATE.md`, `docs/prompts/*.md`) that have since been
> consolidated into `README.md` (architecture/API/deployment) and
> `PROMPTS.md` (all prompts). Kept as-is below for an accurate
> historical record of what changed at each stage.
...
### Session 2026-08-09 (Part 2) — Bug Fixes & Feed UI Upgrade

**Date:** 2026-08-09  
**AI Tool Used:** Antigravity (Gemini 3.6 Flash / Claude Sonnet)  
**Commits:** `221a4cf`, `a4ab04a`, `d5f7a3e`

---

#### Fix #1 — Reddit 429 & VentureBeat 308 (commit `221a4cf`)

**Objective:** Fix two broken RSS sources seen in production logs.

**Root Cause:**
- `Reddit r/LocalLLaMA` RSS returned `429 Too Many Requests` because the httpx client sent a plain bot User-Agent that Reddit blocks.
- `VentureBeat AI` RSS returned `308 Permanent Redirect` because httpx does not follow redirects by default, and the configured URL had a trailing slash.

**Fix:**
- `backend/app/services/topic_discovery.py`: Added `follow_redirects=True` to both `client.get()` call and default `httpx.Client()`. Upgraded `User-Agent` to full Chrome browser string: `Mozilla/5.0 ... AppleWebKit/537.36 ... Chrome/127.0.0.0 Safari/537.36 AetherBot/1.0`.
- `backend/app/core/topic_sources.json`: Removed trailing slash from VentureBeat URL: `https://venturebeat.com/category/ai/feed` (was `...feed/`).

**Verification:** Live HTTP test confirmed both sources return HTTP 200.

**Files Changed:**
- `backend/app/services/topic_discovery.py`
- `backend/app/core/topic_sources.json`
- `backend/scripts/test_topic_discovery.py` (All Stage 11 checks passed)

---

#### Fix #2 — 30-Min Cycle Regeneration Stopped (commit `a4ab04a`)

**Objective:** Fix critical bug where the agent stopped publishing new articles after the first cycle.

**Root Cause:** On each 30-minute cycle, `judge_candidates()` evaluates a batch and stops at `max_accepts=5`. The remaining ~121 unevaluated candidates (not judged because the limit was hit) stayed locked in `SourceCache` for 24 hours. Subsequent cycles found zero eligible candidates (cache dedup filtered everything) and published nothing.

**Fix:**
- `backend/app/services/sources_cache_service.py`: Added `release_unaccepted_from_cache()` which deletes cache rows for ALL non-accepted candidates (both explicitly rejected + unevaluated). Old `release_rejected_from_cache()` retained as backwards-compatible alias.
- `backend/app/services/scheduler.py`: Updated `run_publish_cycle()` to call `release_unaccepted_from_cache()` on all candidates whose URLs are not in the accepted set. Published articles stay locked 24h; everything else re-enters pool next cycle.
...
#### Feature — Feed Batch Dividers, Sidebar Navigator & Prominent Titles (commit `d5f7a3e`)

**Objective:** Upgrade Feed UI with 5-post batch grouping, sticky sidebar index, and guaranteed article title display.

**Changes:**

**Backend:**
- `backend/app/schemas/agent.py`: Added `title: str = ""` field to `FeedPost` schema.
- `backend/app/routes/agent.py`: Updated `/api/agent/feed` to return `title=post.title`.

**Frontend:**
- `frontend/app/lib/api.ts`: Added `title?: string` to `FeedPost` TypeScript type.
- `frontend/app/globals.css`: Added styles for `.batch-divider`, `.batch-badge`, `.batch-time`, `.feed-layout` 2-column grid, `.batch-sidebar`, `.batch-sidebar-item`, `.batch-sidebar-item-header`, `.batch-sidebar-item-time`. Responsive collapse at `max-width: 992px`.
- `frontend/app/feed/page.tsx`:
  - Groups all feed posts into 5-post `BatchGroup[]` chunks, numbered newest-first (`Batch #3`, `Batch #2`, `Batch #1`).
  - Renders glassmorphic `BatchDivider` bar above each group with `⚡ BATCH #N (LATEST)`, formatted date/time, article count.
  - Renders sticky `BatchSidebar` on right with all batch slots. Click → smooth scroll to target batch.
  - `PostCard` now displays `post.title` (from backend) or falls back to text parsing. Always renders bold 24px title — never blank.

**Verification:**
- `python backend/scripts/test_feed_endpoint.py` — All passed.
- `python backend/scripts/test_api_contract.py` — All passed.
