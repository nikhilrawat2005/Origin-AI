...
**`aether-stage4.zip`** — cumulative project ZIP containing everything
through Stage 4 (backend skeleton + all DB models + Next.js frontend
skeleton + `POST /api/agent/init` + docs for stages 0/1/2/3/4).
...
2. Start Stage 5 exactly as planned: `persona.json` +
   `persona_service.py` build a voice profile — no LLM call yet
   (that's Stage 6/8)
